...
- **[#50](issues/050-upstream-issues-track-local-work-only.md) — upstream-targeted issues should track local work, not upstream status**
  An upstream-targeted issue conflates a report owed to upstream with local work blocked on it, which hides pending local work behind `target: ourPLCC/plcc-ng`, caches upstream state that goes stale unnoticed, and defeats any scalar status field when one issue names two upstream refs. Generalizes #22's closing rationale into a rule: an issue tracks work in *this* repo, and upstream status is read on demand rather than recorded.
