...
- **[#31](issues/031-suite-exhausts-disk-and-reports-spurious-failure.md) — `bin/test.bash` exhausts disk and reports a spurious failure**
  On a nearly-full filesystem the suite dies partway through and writes `not ok` against whatever test was running — observed accusing `V4 seq (javascript)`, which passes in every completed run — while the truncated output still counts cleanly with `grep -c`. Issue #27's `plcc-rep -s` removes the per-test whole-tree copy that causes it; the misleading report is worth fixing separately.
