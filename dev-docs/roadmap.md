...
- **[#38](issues/038-plcc-ng-follow-set-omits-nullable-tail.md) — FOLLOW set omits the nullable tail, breaking empty alternatives**
  `build_follow_sets` adds FIRST of only the *next* symbol instead of walking forward through nullable ones, so an empty alternative followed by a `**=` nonterminal loses parse-table entries while `plcc-ll1` still reports `is_ll1: true`. Measured on OBJ's `<Ext:Ext0>`: `class static x = 3 end` parses, `class field x end` does not. Worked around in `src/OBJ/grammar.plcc` with a non-nullable `<ClassBody>`.
